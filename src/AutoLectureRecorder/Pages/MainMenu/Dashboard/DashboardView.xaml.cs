...
public partial class DashboardView : ReactiveUserControl<DashboardViewModel>
{
    public DashboardView()
    {
        ...
        this.WhenActivated(disposables =>
        {
            ...
            this.OneWayBind(ViewModel, 
                    vm => vm.TodaysLectures, 
                    v => v.TodaysLecturesListView.ItemsSource)
                .DisposeWith(disposables);

            this.WhenAnyValue(v => v.ViewModel!.LecturesScheduler.NextScheduledLectureTimeDistance)
                .Subscribe(UpdateNextLecture)
                .DisposeWith(disposables);

            // Text bindings
            this.OneWayBind(ViewModel, 
                    vm => vm.RegistrationNumber, 
                    v => v.RegistrationNumTextBox.Text,
                    rn => CultureInfo.CurrentCulture.TextInfo.ToTitleCase(rn))
                .DisposeWith(disposables);

            // Visibilities
            this.OneWayBind(ViewModel, 
                    vm => vm.AreLecturesScheduledToday, 
                    v => v.TodaysLecturesScrollViewer.Visibility,
                    value => value ? Visibility.Visible : Visibility.Collapsed)
                .DisposeWith(disposables);

            this.OneWayBind(ViewModel, 
                    vm => vm.AreLecturesScheduledToday, 
                    v => v.TodaysLecturesErrorTextBlock.Visibility,
                    value => value ? Visibility.Collapsed : Visibility.Visible)
                .DisposeWith(disposables);

            // Commands
            this.BindCommand(ViewModel, 
                    vm => vm.NavigateToCreateLectureCommand, 
    ...
    {
        if (timeDiff == null || ViewModel!.LecturesScheduler.NextScheduledLecture == null)
        {
        ...
        this.NextLectureSubjectNameTextBlock.Text = ViewModel.LecturesScheduler.NextScheduledLecture.SubjectName;
