...
namespace AutoLectureRecorder.Infrastructure.StartupManager;

#pragma warning disable CA1416 // Validate platform compatibility
public class WindowsStartupManager : IStartupManager
{
    private readonly byte[] _startupEnabledValue = { 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    private string StartupEnabledValueString => BitConverter.ToString(_startupEnabledValue).Replace("-", "");
    private readonly byte[] _startupDisabledValue = { 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    private string StartupDisabledValueString => BitConverter.ToString(_startupDisabledValue).Replace("-", "");
    ...
    public IObservable<bool> IsStartupEnabledObservable => Observable
    ...
    public bool IsStartupEnabled()
    {
        ...
        using (RegistryKey? key = Registry.CurrentUser.OpenSubKey(ApprovedStartupRegistryPath))
        {
            ...
            foreach (string startupAppName in startupAppNames)
            {
                ...
                if (value.First() == _startupEnabledValue.First())
                {
